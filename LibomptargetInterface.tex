...
enum tgt_map_type {
  ...
  OMP_TGT_MAPTYPE_MAPPED_PTR = 0x0010,
  OMP_TGT_MAPTYPE_TARGET_VAR = 0x0020,
  OMP_TGT_MAPTYPE_USE_DEVICE_PTR = 0x0040,
  OMP_TGT_MAPTYPE_PRIVATE = 0x0080
};
\end{lstlisting}

The attributes \code{OMP_TGT_MAPTYPE_TO} to \code{OMP_TGT_MAPTYPE_DELETE} follow the map types in the OpenMP 4.5 specification. \code{OMP_TGT_MAPTYPE_MAPPED_PTR} indicates the presence of a mapped pointer, for which the runtime maps the pointer variable as well as the pointee variable (namely, the variable pointed to by the pointer). The attribute also instructs the runtime to initialize the value of the pointer on the device with the base device address of the pointee mapped variable. \code{OMP_TGT_MAPTYPE_TARGET_VAR} instructs the runtime to add the base device address of the mapped variable to the target var pointer list.  \code{OMP_TGT_MAPTYPE_USE_DEVICE_PTR} instructs the runtime to returns the base device address of the mapped variable in the corresponding \code{base_args} entry. When used in conjunction with the \code{OMP_TGT_MAPTYPE_MAPPED_PTR} attribute, it returns the base device address of the pointer mapped  variable. \code{OMP_TGT_MAPTYPE_PRIVATE} informs the runtime that the operation is for a private variable instead of a mapped variable. Firstprivate operations are indicated by using the  \code{OMP_TGT_MAPTYPE_TO}  attribute in conjunction with the private attribute.  The fields in the interface are used in two different ways depending on the presence of the \code{OMP_TGT_MAPTYPE_MAPPED_PTR} attribute.
%
\begin{itemize}
...
%
\end{itemize}
Regardless of the interface, zero-length array references are identified by their \code{args_size} values of zero. By definition, zero-length array have no offset.
...
foo(){
  ...
  #pragma omp target data map(pA[S:M], s1)
  { 
